...
module inverted_outsets(x, wall_thickness, width, margin) {
  ...
  translate([wall_thickness, 0, 0])
    outset(extra - margin / 2, wall_thickness);
  translate([x + wall_thickness - extra + margin / 2, 0, 0])
    outset(extra - margin / 2, wall_thickness);
  translate([width, 0, 0]) {
    outsets(x - width * 2, wall_thickness, width, margin);
  }
}
...
module box_bottom_piece (x, y, thickness, outset_width, outset_margin) {
  union () {
    // Body
...
module box_side_piece (x, y, thickness, outset_width, outset_margin) {
  union () {
    translate([thickness, thickness, 0]) {
      cube([x, y, thickness]);
    }
...
module box (width,
            ...
            outset_margin = 0.1,
            piece_margin = 5) {
  box_bottom_piece(width, breadth, wall_thickness, outset_width, outset_margin);
  ...
  translate([piece_offset(width, wall_thickness, piece_margin, 1), 0])
    box_side_piece(width, height, wall_thickness, outset_width, outset_margin);
  ...
  translate([piece_offset(width, wall_thickness, piece_margin, 1),
    ...
    box_side_piece(breadth, height, wall_thickness, outset_width, outset_margin);
